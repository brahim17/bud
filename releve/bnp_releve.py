...
def get_tier_from_rules_and_memo(rules_tier_csv, str_memo):
    ...
    if len(list_tier) == 1:
        ret_val = list_tier.pop()
    #    else :
    #        print(str_memo, list_tier)
...
def get_tiers_from_rules_and_memo_series(rules_csv, memo_series):
    ...
    ret_val = ret_val.transform(lambda x: get_tier_from_rules_and_memo(rules_tier_csv, x))
...
def extract_bnp_releve(year, init_solde):
    ...
    df_bnp['tier'] = get_tiers_from_rules_and_memo_series(
