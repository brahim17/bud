...
def plot_pie(df_bnp, category):

    # delete columns : solde
    filter_periode = (df_bnp['date'] >= '2020-01-01') & (df_bnp['date'] <= '2020-12-31')
    filter_category = True #(df_bnp[category] != 'AMAZON')

    df_bnp = df_bnp[filter_category & filter_periode]

    df_bnp.to_csv(directory_path + "ing-filer-1.0.csv")

    plot_pie_amount(df_bnp, 'amount', category)


def plot_pie_amount(df_bnp, y_tab, group_by):

    gk = df_bnp.groupby(group_by).sum()
    print(gk.info())


    gk[y_tab] = gk[y_tab].abs()

    print(gk)
    gk.plot.pie(y=y_tab, x=group_by,
                autopct='%1.1f%%',
                startangle=90)
    plt.show()
